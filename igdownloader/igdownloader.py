...
def keep_ext_alive(d): d.execute_script("if (chrome && chrome.runtime) chrome.runtime.sendMessage({action: 'keepAlive'});")
...
def proxy_ext(h,p,u,pw):	# manifest v3 chrome extension
	tmp=mkdtemp()
	try:
		ext=join(tmp,'extension');makedirs(ext)	# ext path done, now lets create manifest and background extension files ⬇⬇
		open(join(ext,'manifest.json'),'w').write('{"manifest_version":3,"name":"P","version":"1","permissions":["proxy","webRequest","webRequestAuthProvider"],"host_permissions":["<all_urls>"],"background":{"service_worker":"background.js"}}')
		open(join(ext,'background.js'),'w').write(f'const c={{mode:"fixed_servers",rules:{{singleProxy:{{scheme:"http",host:"{h}",port:{p}}},bypassList:["localhost","127.0.0.1"]}}}};const a={{username:"{u}",password:"{pw}"}};function s(){{chrome.proxy.settings.set({{value:c,scope:"regular"}},()=>{{}})}}chrome.runtime.onStartup.addListener(s);chrome.runtime.onInstalled.addListener(s);chrome.webRequest.onAuthRequired.addListener((d,b)=>b({{authCredentials:a}}),{{urls:["<all_urls>"]}},["asyncBlocking"]);chrome.runtime.onMessage.addListener((m,x,r)=>m.action==="keepAlive"&&r({{status:"ok"}}));s();')
		dbg(f'{DARK}[debug] ext: {ext}{RESET}')
		return ext
	except Exception as e:print(f'{RED}[error] ext: {e}{RESET}');return None
...
def drv(proxy=None):	# currently only chrome driver is supported
	...
	if proxy:
		try:
			...
			if all([parsed.hostname,parsed.port,parsed.username,parsed.password]):
				proxy_host=parsed.hostname;proxy_port=parsed.port;proxy_user=parsed.username;proxy_pass=parsed.password
				dbg(f'{DARK}[debug] proxy: {proxy_host}:{proxy_port}{RESET}')
				ext=proxy_ext(proxy_host,proxy_port,proxy_user,proxy_pass)
				if not ext:raise Exception('error creating extension')
				o.add_argument(f'--load-extension={ext}')
				o.add_argument('--disable-extensions-except='+ext)
				o.add_argument('--disable-extensions-file-access-check')
				o.add_argument('--enable-extension-activity-logging')
			else:
				dbg(f'{DARK}[debug] proxy no auth{RESET}')
...
def click_post(d,element):
	try:
		if 'profile picture' in (element.find_element(By.TAG_NAME,'img').get_attribute('alt') or ''):return 'Profile picture',''	# dont click if pp + desc assigned
						...
						if batch_count%BATCH==0:
							...
							if a.proxy:
								try:
									...
									if all([parsed.hostname,parsed.port,parsed.username,parsed.password]):keep_ext_alive(d)
								except:pass
...
def main():
	...
	ap.add_argument('--proxy',help='proxy with auth: http://user:pass@ip:port')
	...
	if a.proxy:
		try:
			...
			if parsed.hostname and parsed.port:
				...
				dbg(f'{DARK}[debug] proxy setup for chrome and requests (note: ffmpeg configures proxy via env vars){RESET}')
			...
			try:
				if a.proxy:
					try:
						parsed=urlparse(a.proxy)
						if all([parsed.hostname,parsed.port,parsed.username,parsed.password]):
							dbg(f'{DARK}[debug] waiting 10s for extension...{RESET}')
							sleep(10);keep_ext_alive(d)
					except:pass
